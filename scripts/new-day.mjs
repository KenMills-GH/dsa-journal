...
// NOTE: `dsa/answers` and answer files removed — script no longer creates them.
const answersPath = null;

const nowDate = new Date().toISOString().split("T")[0];
const author = process.env.USER || process.env.USERNAME || "You";

// NOTE: no longer create a top-level file in `dsa/`.
// The practice, notes, and answers files are created instead.
...
// answers scaffold intentionally omitted
...
if (gitCommit && !dryRun) {
  try {
    const filesToAdd = [notesPath, practicePath]
